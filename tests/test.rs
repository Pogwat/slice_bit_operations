...
fn ctz_popcnt() {
    ...
    assert_eq!(sidx,0);
    assert_eq!(eidx,3);
    assert_eq!(array.bit_iter().ctz(), 4*8-1-1-2-1);
    assert_eq!(array.biter(5..).ctz(), 4*8-5-1-2-1);
    assert_eq!(array.biter(5..=7).ctz(), 3);
    assert_eq!(array.biter(0..).ctz() + array.biter(0..).popcnt(),4*8);
    assert_eq!(array.biter(0..).popcnt(), 1+1+2+1);
    assert_eq!(array.biter(0..=7).popcnt(), 1); //Array[0]
    assert_eq!(array.biter(7+1..=14+1).popcnt(), 1); //Array[1]
    assert_eq!(array.biter(14+1+1..=21+1).popcnt(), 2); //Array[2]
    assert_eq!(array.biter(21+1+1..=28+1).popcnt(), 1); //Array[3]
}
